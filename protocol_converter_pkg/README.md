...
- **Structured Outputs 往返**：Responses `text.format` ↔ Chat `response_format` 的 `json_schema` 格式正确互转，支持 `name`/`schema`/`strict` 字段完整映射
...
│   └── engine.py                # 核心转换引擎
├── tests/
│   └── test_protocol_converter.py   # 229 个单元测试
├── examples/
