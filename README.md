...
| 版本 | 日期 | 说明 |
|------|------|------|
| **v1.36.0** | 2026-05-18 | 修复 Responses → Chat 转换时参数过滤问题：移除 store/include/prompt_cache_key/safety_identifier/prompt_cache_retention 等不支持字段；优化流式结束信号处理避免重复 |
...
**当前版本**：`1.36.0`（同步更新于 `pyproject.toml` 和 `protocol_converter/__init__.py`）
